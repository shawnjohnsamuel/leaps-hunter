...
**Status:** approved 2026-09-03, **all 7 phases complete as of 2026-09-04** · **Governing doc:** [`framework/v7.md`](../framework/v7.md)
...
| **3** ✅ | Engine quant — pricing, sizing, feasibility | **Complete 2026-09-03** — `engine/{optmodel,sizing,portfolio}.py`, 155 passing tests total across the engine. Black-Scholes reproduces the real Phase 0 CRM quote's mark and delta; the classic 60/40 Kelly coin-flip reproduces the textbook f*=0.2 exactly; `min_feasible_nav` (ADR 0013) reproduces the $3.19M finding to the dollar; the S15 roll formula matches a hand-worked example intermediate-value-for-intermediate-value. **The engine build (Phases 2-3) is complete.** |
| **4** ✅ | State plane and calibration ledger | **Complete 2026-09-03** — `state/{config,watchlist,macro-latest,calibration}.json` in `leaps-hunter-data` (branch `v7-migration`), [`docs/storage-schema-v7.md`](storage-schema-v7.md). Seed watchlist re-verified against live data: 3 names confirmed M1, 3 M2, 3 M3 — one of the three M3 names has no current dislocation, one is hard-gated by an earnings date 7 days out. **One human action required: set `state/config.yaml`'s `portfolio.account_ref`** before any real run — never inferred, see the file's own comment. |
| **5** ✅ | Three skills | **Complete 2026-09-03** — `.claude/skills/{weekly-review,daily-screen,bench-check}/SKILL.md`. Frozen fixture-day dry run done for real, not synthetically: live macro state (R=4, RESTRICTED, no hard gates) written to `state/macro-latest.json`, and CRWD's panic pattern found to fully clear today (+0.29% NTM revision — zero consensus cut) against real, freshly-fetched data. Also patched a real gap: `engine.scoring` (§11's aggregator) didn't exist until this phase needed it. 164 tests total. |
| **6** ✅ | Cloud routines and release step | **Complete 2026-09-04** — 2 routines live (weekly Sundays, daily weekdays, both 10:15 ET). Release step is git tags (`v7.0.0`–`v7.0.2`) on `main`, not a dedicated `scripts/release.py` — merge-then-tag serves the same "pin what production reads" purpose with less machinery. First unattended runs observed end to end, including two real infrastructure bugs found and fixed live: a sensitive-file classifier hang (ADR-worthy but folded into the fix commit) and FRED/Alpha Vantage/SEC-EDGAR being unreachable from the cloud sandbox ([ADR 0014](decisions/0014-macro-fetch-desktop-only.md)), which split macro/NTM fetching into a new desktop-only `macro-refresh` skill. |
| **7** ✅ | Migrate the public plane | **Complete 2026-09-04** — `sanitize.py` v2 (allowlist redesigned around v7's schema; `account_ref`/`nav_at_run` never named, not merely filtered) + 13 leak tests (`scripts/tests/test_sanitize.py`, run in CI) · `render_email.py` v2 · `app/lib/data.ts` + `ui.tsx` (mechanism tags, macro/R badges, no regime verdict or tiers) · 4 fictional v7 demo days (`data-demo/`, one per result type) · legacy-mode rendering verified live in the browser — the 5 real v6.1 days and a synthetic v7 day render correctly side by side on the same dashboard. `leak-check.yml` updated to validate both schema versions. Real v7 daily records have NOT yet been published to `public-data/` — that's a separate "publish real output" decision, not an infrastructure one. |
